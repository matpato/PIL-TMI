...
import numpy as np
import time
from database import check_if_pair_exist, insert_row
...
def calculate_sim_it1_it2(it1, it2, e1, name_prefix):
    ...
    if check_if_pair_exist(it1, it2) is False:
        ...
        try:
            ...
            insert_row(it1.item(), it2.item(), items_sim_resnik, items_sim_lin, items_sim_jc)
...
def calculate_semantic_similarity(onto_ids: object, name_prefix: object, train: object) -> object:
    ...
    for it1 in onto_ids:
        ...
        pool.starmap_async(calculate_sim_it1_it2,
                           [(it1, it2, e1, config.host, config.user, config.password, config.database) for it2 in train]).get()
        end = time.time()
        print(end - start)
        pool.close()
        count += 1
